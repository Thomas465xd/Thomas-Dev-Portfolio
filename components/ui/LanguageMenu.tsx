...
export default function LanguageMenu({
	...
	const handleLanguageSelect = (languageCode: string): void => {
		...
		document.cookie = `NEXT_LOCALE=${languageCode};expires=${expires};path=/`;
		...
		if (
			...
			router.push("/" + languageCode + currentPathname);
		} else {
			router.push(
				currentPathname.replace(`/${currentLocale}`, `/${languageCode}`),
			);
		}
