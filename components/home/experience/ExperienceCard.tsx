...
export default function ExperienceCard({ type, title, businessName, location, description, date }: ExperienceCardProps) {
    return (
        <article className="mt-12 group relative overflow-hidden rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/70 dark:bg-gray-900/70 backdrop-blur-sm hover:bg-white/90 dark:hover:bg-gray-900/90 transition-all duration-300 hover:shadow-xl hover:shadow-gray-200/20 dark:hover:shadow-gray-900/40 hover:-translate-y-1 p-6 space-y-5">
            ...
            {/* Type & Date badges */}
            <div className="flex-between flex-wrap relative z-10">
                ...
                <span className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-gradient-to-r from-gray-500/10 to-blue-500/10 text-gray-600 dark:text-gray-400 border border-gray-200/50 dark:border-gray-500/30 backdrop-blur-sm">
                    <div className="w-1.5 h-1.5 bg-gray-500 rounded-full mr-2 animate-pulse"></div>
                    {date}
                </span>
            </div>
